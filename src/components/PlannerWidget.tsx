...
export function PlannerWidget() {
  ...
  const loadEvents = useCallback(async () => {
    // Only fetch if we have a user, otherwise clear events
    if (!userId) {
      setEvents([]);
      setIsLoading(false);
      return;
...
}, [isLoaded, userId, loadEvents]); // Ensure userId is always here
  // --- ACTIONS ---
          ...
          <div key={event.id} className="relative pl-8 group animate-in fade-in slide-in-from-left-2 duration-300">
            ...
            <div className="flex justify-between items-start">
              <div className={`flex flex-col ${event.completed ? 'opacity-40' : ''}`}>
                ...
                <span className="text-[10px] font-bold text-zinc-600">{event.subtitle}</span>
              </div>
